...
{
	private static final long serialVersionUID = 8644812107248396435L;

	private MenuParams p;
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
    	...
    	try 
    	{
	    	...
	    	else  // New record for db, update and delete records to/from db
	    	{
	    		...
	    		String[] formData = p.formData, logData = new String[formSize-1];
	    		
	    		for(int i=0; i<formSize; i++)
    			{
	    			if(i < 3) logData[i] = formData[i];
	    			else if (i>3) logData[i-1] = formData[i];
	    			else i++;
    			}
