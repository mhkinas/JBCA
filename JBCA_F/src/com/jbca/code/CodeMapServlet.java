...
@WebServlet(urlPatterns = "/codeMapServlet", 
			loadOnStartup = 1,
			asyncSupported = true)
@MultipartConfig (fileSizeThreshold = 1024*1024, 
				 maxFileSize = 1024*1024*5, 
				 maxRequestSize = 1024*1024*5*5)
public class CodeMapServlet extends HttpServlet
{
	private static final long serialVersionUID = 1516583676360852833L;

	private CodeMapParams p;
    	...
    	try 
    	{
	    	...
	    	else  // New record for db, update and delete records to/from db
	    	{
	    		...
	    		if(dbQuery) {
	    			...
	    			if(!p.logBean.getErrorString().equals("null errorString"))
	    				System.out.println(p.logBean.getErrorString());
	    			...
	    			if(!isInsert)
	    			{
		       			if(p.tableData[0] != null && p.tableData[0][4] != null)
		       				p.tableData[0][4] = p.dateFormatter.changeDateFormat((String) p.tableData[0][4]);
		       			else
