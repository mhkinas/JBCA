	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
	    	...
	    	else  // New record for db, update and delete records to/from db
	    	{
	    		...
	    		if(dbQuery) {
	    			...
	    			if(isInsert || isUpdate)
	    			{
	    				...
	    				if(p.reader != null)
	    				{
		    				p.execFileData.setReader(p.reader);
	    		...
	    		if(messages[1].equals(p.execTableWR.getDefMsgScr()))
	    			messages[1] = " ";
	    		messages[1] += msgStr+", sqlString: \""+p.execTableWR.getQueryString()+"\", logSQL:\""+p.logBean.getQueryStr()+"\"";
    	...
    	catch(Exception ex) 
    	{
	  	...
	  	finally 
	  	{
            ...
            if (p.writer != null) p.writer.close();
