...
<%@ page session="false"%>
<%@ page import="com.jbca.exchange.CurrencyParams,com.jbca.base.BaseProcesses, 
				 com.sdb.utility.JsonConverter, 
				 com.sdb.utility.ArrayToString, 
				 java.util.Arrays"%>

<%
	CurrencyParams p = new CurrencyParams();
	p.setLangLoc();
	
	BaseProcesses process = new BaseProcesses(request, response, p);
	process.checkSes4JSP();

	String jsObjData = null, servletName = "currencyServlet", errorString = null,
	...
	try {
		...
		leftSideStr = p.pageBean.getPage(null, false, new String[] {"Para-Sidebar","/JBCA_F/page/"});
				...
				<form method="post" id="yeniK" class="form collapse" enctype="multipart/form-data">
					<div class="form-group">
						<table class="table table-borderless">
							<tbody>
								<tr>
									<td><label for="y_1" style="margin-top: 7px;"><%=p.labelNames[1]%>:</label></td>
									<td><input type="<%=p.inputTypes[1]%>" class="form-control form-control-sm" name="<%=p.fieldNames[1]%>" 
											id="y_1" placeholder="<%=p.labelNames[1]%> giriniz." required></td>
									<td><label for="y_2" style="margin-top: 7px;"><%=p.labelNames[2]%>:</label></td>
									<td><input type="<%=p.inputTypes[2]%>" class="form-control form-control-sm" name="<%=p.fieldNames[2]%>" 
											id="y_2" placeholder="<%=p.labelNames[2]%> giriniz." required></td>
									<td>&nbsp;&nbsp;&nbsp;</td>
									<td>&nbsp;&nbsp;&nbsp;</td>
									<td>&nbsp;&nbsp;&nbsp;</td>
									<td>&nbsp;&nbsp;&nbsp;</td>
								...
								<tr>
									<td><button type="submit" class="btn btn-outline-info btn-sm" 
												onclick='sendFormWF("/<%=p.contextRoot%>/<%=servletName%>", "yeniK","insert")'>Kaydet</button></td>
				...
				<form method="post" id="araK" class="form collapse" enctype="multipart/form-data">
					<div class="form-group">
						<table class="table table-borderless">
							<tbody>
								<tr>
									<td><label for="s_1" style="margin-top: 7px;"><%=p.labelNames[0]%>:</label></td>
									<td><input type="<%=p.inputTypes[0]%>" class="form-control form-control-sm" id="s_1"
										name="<%=p.selectFields[0]%>" placeholder="<%=p.labelNames[0]%> giriniz."></td>
									<td><label for="s_2" style="margin-top: 7px;"><%=p.labelNames[1]%>:</label></td>
									<td><input type="<%=p.inputTypes[1]%>" class="form-control form-control-sm" id="s_2"
										name="<%=p.selectFields[1]%>" placeholder="<%=p.labelNames[1]%> giriniz."></td>
									<td>&nbsp;&nbsp;&nbsp;</td>
									<td>&nbsp;&nbsp;&nbsp;</td>
									<td>&nbsp;&nbsp;&nbsp;</td>
									<td>&nbsp;&nbsp;&nbsp;</td>
								...
								<tr>
									<td><button type="button"
											...
											onclick='sendFormWF("/<%=p.contextRoot%>/<%=servletName%>","araK","select")'>Ara</button></td>
									<td><button type="reset"
											class="btn btn-outline-secondary btn-sm">İptal</button></td>
									<td>&nbsp;</td>
									<td>&nbsp;</td>
									...
									<tr>
										<td>
											<button type="submit" class="btn btn-outline-info btn-sm"
												onclick='sendFormWF("/<%=p.contextRoot%>/<%=servletName%>","guncelleK","update")'>Kaydet</button>
											&nbsp;
											<button type="button" class="btn btn-outline-danger btn-sm"
												onclick='sendFormWF("/<%=p.contextRoot%>/<%=servletName%>","guncelleK","delete")'>Sil</button>
											&nbsp;
