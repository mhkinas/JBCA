...
<%@ page session="false"%>
<%@ page import="com.jbca.reference.MessageTypeParams,com.jbca.base.BaseProcesses, 
				 com.sdb.utility.JsonConverter, 
				 com.sdb.utility.ArrayToString, 
				 java.util.Arrays"%>

<%
	MessageTypeParams p = new MessageTypeParams();
	p.setLangEN();
	
	BaseProcesses process = new BaseProcesses(request, response, p);
	process.checkSes4JSP();

	String jsObjData = null, servletName = "messageTypeServlet", errorString = null, qsql = null, menuString = null, 
			leftSideStr = null, excelCols = null;
...
<body>
	...
	<div class="row"> 
      <div class="col-md-12">
		<a href="#araK" class="btn btn-outline-dark" data-toggle="collapse">Search <%=p.jspTitle%></a>
				...
				<table class="table table-borderless">
					<tbody>
						<tr>
							...
							<td><input type="<%=p.inputTypes[0]%>" class="form-control form-control-sm" id="s_1" name="<%=p.selectFields[0]%>" 
									   placeholder="Enter <%=p.labelNames[0]%>"></td>
							<td><label for="s_2" style="margin-top: 7px;"><%=p.labelNames[1]%>:</label></td>
							<td><input type="<%=p.inputTypes[1]%>" class="form-control form-control-sm" id="s_2" name="<%=p.selectFields[1]%>"
									   placeholder="Enter <%=p.labelNames[1]%>"></td>
							<td>&nbsp;&nbsp;&nbsp;</td>
						</tr>
						<tr></tr>
						<tr>
							<td><button type="button" class="btn btn-outline-info btn-sm"
									onclick='sendSelect("/<%=p.contextRoot%>/<%=servletName%>","araK")'>Search</button></td>
							<td><button type="reset" class="btn btn-outline-secondary btn-sm">Cancel</button></td>
							<td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td>
							<td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td>
						</tr>
									...
									<tr>
									  <td>
										<button type="submit" class="btn btn-outline-info btn-sm"
											onclick='sendUpdate("/<%=p.contextRoot%>/<%=servletName%>","guncelleK")'>Save</button>
											 &nbsp;
										<button type="button" class="btn btn-outline-danger btn-sm"
											onclick='sendDelete("/<%=p.contextRoot%>/<%=servletName%>","guncelleK")'>Delete</button>
											 &nbsp;
