...
<%@ page session="false"%>
<%@ page import="com.jbca.log.LogParams,com.jbca.base.BaseProcesses, 
...
<%
	...
	String jsObjData = null, servletName = "logServlet", selectLists = null, errorString = null, 
			qsql = null, menuString = null, codeStrMW90 = null, leftSideStr = null, excelCols = null;
	...
	try {
		...
		leftSideStr = p.pageBean.getPage(null, false, new String[] {"Logs-Sidebar","/JBCA_F/page/"});
...
<body>
						...
						<table class="table table-borderless">
							<tbody>
								<tr>
									...
									<td><input type="<%=p.selectITs[0]%>" class="form-control form-control-sm" id="s_1"
										name="<%=p.selectFields[0]%>" placeholder="Enter <%=p.selectLabels[0]%>"></td>
									<td><label for="s_2" class="col-form-label-sm"><%=p.selectLabels[1]%>:</label></td>
									<td><input type="<%=p.selectITs[1]%>" class="form-control form-control-sm" id="s_2"
										name="<%=p.selectFields[1]%>" placeholder="Enter <%=p.selectLabels[1]%>"list="forms"></td>
										<datalist id="forms"></datalist>
									<td><label for="s_3" class="col-form-label-sm"><%=p.selectLabels[2]%>:</label></td>
									<td><input type="<%=p.selectITs[2]%>" class="form-control form-control-sm" id="s_3"
										name="<%=p.selectFields[2]%>" placeholder="Enter <%=p.selectLabels[2]%>" list="types"></td>
										<datalist id="types"></datalist>
									<td><label for="s_4" class="col-form-label-sm"><%=p.selectLabels[3]%>:</label></td>
									<td><input type="<%=p.selectITs[3]%>" class="form-control form-control-sm" id="s_4"
										name="<%=p.selectFields[3]%>" placeholder="Enter <%=p.selectLabels[3]%>" list="tables"></td>
										<datalist id="tables"></datalist>
								</tr>
								<tr>
									<td><label for="s_5" class="col-form-label-sm"><%=p.selectLabels[4]%>:</label></td>
									<td><input type="<%=p.selectITs[4]%>" class="form-control form-control-sm" id="s_5"
										name="<%=p.selectFields[4]%>" placeholder="Enter <%=p.selectLabels[4]%>" list="codes"></td>
									<datalist id="codes"></datalist>
									<td><label for="s_6" class="col-form-label-sm"><%=p.selectLabels[5]%>:</label></td>
									<td><input type="<%=p.selectITs[5]%>" class="form-control form-control-sm" id="s_6"
										name="<%=p.selectFields[5]%>" placeholder="Enter <%=p.selectLabels[5]%>"></td>
									<td><label for="s_7" class="col-form-label-sm"><%=p.selectLabels[6]%>:</label></td>
									<td><input type="<%=p.selectITs[6]%>" class="form-control form-control-sm" id="s_7"
										name="<%=p.selectFields[6]%>" placeholder="Enter <%=p.selectLabels[6]%>"></td>
									<td>&nbsp;</td>
									<td>&nbsp;</td>
								</tr>
								<tr>
					...
					<table
						...
						<thead>
							...
							<tr>
								...
								<th onclick="sortByProp('<%=p.fieldNames[5]%>')" style="display:none;"><%=p.labelNames[5]%></th>
	...
	<%}%>		
		...
		selInpIds = {searchR:[{id:"s_2", name:"form_name"}, {id:"s_3", name:"type_name"}, {id:"s_4", name:"table_name"}, 
