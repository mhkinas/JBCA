	  ...
	  for(let i=0; i<m; i++)
	  {
		if(window.dataList[i][window.fieldNames[0]] == id)
		{
		  for(let j=0; j<k; j++)
		  {
			 ...
			 {	 
				tableStr += "<tr><td><label for=\"g_"+(j+1)+"\" class=\"col-form-label-sm\">"+window.labelNames[j]+":</label></td>"+
							"<td colspan="+(2*mCols-1)+"><textarea  class=\"form-control form-control-sm\" id=\"g_"+(j+1)+"\""+
				...
				if(isLog)
				{
					...
					logStr(logDataStr);
					tableStr += ">"+logDataStr+"</textarea></td></tr>";
				}		
				else
					tableStr += ">"+window.dataList[i][window.fieldNames[j]]+"</textarea></td></tr>";
			 }
			 else
			 {
				if((j % mCols) == 0) // is even 
				 ...
				 tableStr += "<td><label for=\"g_"+(j+1)+"\" class=\"col-form-label-sm\">"+window.labelNames[j]+":</label></td>"+
				 ...
				 if(window.listN4Modal != undefined && window.listN4Modal)
				 {
					 for(h=0; h < n; h++)
					 {
						...
						{
							tableStr += " list=\""+window.listN4Modal[h].id+"\"></td><datalist id=\""+
										window.listN4Modal[h].id+"\"></datalist>";
							isList = true;
						}
					 }
				 }
				 ...
				 if(j > 0 && ((j % mCols) == (mCols-1) || j == k-1 || window.inputTypes[j] == "textarea")) // is odd or last
					tableStr += "</tr>";
			 }					
		  }
		  document.getElementById("modalTb").innerHTML = tableStr;
