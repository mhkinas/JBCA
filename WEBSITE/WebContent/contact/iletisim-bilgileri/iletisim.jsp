...
<%
	...
	String headLink = null, servletName = "contactServlet", codeStrMW90 = null, codeStrBW = null, pageName = "iletisim";

	try {
		centerPageStr = linPageBean.getLinPage(pageName);
...
<head>
  ...
  <% 
  	if(codeStrBW != null) { 
  %>
  <script><%=codeStrBW %></script>
  <% } %>
  <% 
  	if(codeStrMW90 != null) {
  %>
  <style type="text/css"><%=codeStrMW90 %></style>
  <% } %>
</head>

<body>

<%  if(topPageStr != null) out.write(topPageStr); 

	if(menuString != null) out.write(menuString); %>
	...
	<div class="container">
	  <div class="row">
		<div class="col">
		<%=centerPageStr %>
		<form method="post" id="yeniK" class="form" enctype="multipart/form-data">
					...
					<tbody>
						<tr>
							...
							<td><input type="<%=p.inputTypes[3]%>" class="form-control form-control-sm" name="<%=p.fieldNames[3]%>" id="y_3"
								placeholder="<%=p.labelNames[3]%> giriniz."></td>
						</tr>
						<tr>
							...
							<td colspan="7"><textarea class="form-control form-control-sm" name="<%=p.fieldNames[4]%>" id="y_4"
								placeholder="<%=p.labelNames[4]%> giriniz." required rows="10" cols="120"></textarea></td>
						</tr>
						<tr>
							<td>&nbsp;</td>
	...
	<%	if(bottomPageStr != null) out.write(bottomPageStr); %>
	...
	<script>
		var jsObjData, isLocale, isWithRefs=false;
